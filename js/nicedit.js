...
var bkLib = {
  ...
  onDomLoaded: function (fireThis) {
    ...
    if (document.addEventListener) {
      ...
      document.write(
        "<style>.nicEdit-main p { margin: 0; }</style><scr" +
          "ipt id=__ie_onload defer " +
...
var nicEditorInstance = bkClass.extend({
  ...
  construct: function (e, options, nicEditor) {
    ...
    if (isTextarea || this.options.hasPanel) {
      ...
      editorElm.setStyle(
        ie7s ? { height: newY + "px" } : { overflow: "hidden" }
      );
      this.elm = editorElm;
    }
    ...
    if (r.startContainer) {
      ...
      if (r.cloneContents().childNodes.length == 1) {
        for (var i = 0; i < contain.childNodes.length; i++) {
          ...
          if (
            r.compareBoundaryPoints(Range.START_TO_START, rng) != 1 &&
...
var nicEditorPanel = bkClass.extend({
  ...
  findButton: function (itm) {
    for (var i = 0; i < this.panelButtons.length; i++) {
      if (this.panelButtons[i].name == itm) return this.panelButtons[i];
    }
  },
...
var nicEditorButton = bkClass.extend({
  ...
  checkNodes: function (e) {
    ...
    if (this.options.css) {
      for (itm in this.options.css) {
        if (
          elm.getStyle(itm, this.ne.selectedInstance.instanceDoc) ==
  ...
  key: function (nicInstance, e) {
    if (
      this.options.key &&
      e.ctrlKey &&
      String.fromCharCode(e.keyCode || e.charCode).toLowerCase() ==
        this.options.key
    ) {
      this.mouseClick();
...
var nicEditorPane = bkClass.extend({
  ...
  position: function () {
    if (this.ne.nicPanel) {
      ...
      var newLeft =
        panelPos[0] +
        parseInt(panelElm.getStyle("width")) -
    ...
    if (!this.isDisabled) {
      if (this.pane && this.pane.pane) {
        ...
        this.pane = new nicEditorPane(
          this.contain,
          this.ne,
          { width: this.width || "210px", backgroundColor: "#fff" },
          this
        );
        this.addPane();
  ...
  addPane: function () {
    ...
    for (var g in colorList) {
      ...
      var colorBorder = new bkElement("DIV")
        .setStyle({ border: "1px solid " + boxborderclr })
      ...
      var colorInner = new bkElement("DIV")
        ...
        .addEvent("mouseout", this.off.closure(this, colorBorder, boxborderclr))

    ...
    commands: {
              ...
              .add(
                plugin.day(
                  plugin._applyMonthsOffset(
                    plugin.add(
                      plugin.newDate(inst.drawDate),
                      1 - inst.options.monthsToStep,
                      "m"
                    ),
                    inst
                  ),
      ...
      current: {
        ...
        enabled: function (inst) {
          ...
          return (
            (!minDate || curDate.getTime() >= minDate.getTime()) &&
      ...
      today: {
        ...
        enabled: function (inst) {
          ...
          var maxDate = inst.get("maxDate");
          return (
            (!minDate || plugin.today().getTime() >= minDate.getTime()) &&
      ...
      prevWeek: {
        ...
        enabled: function (inst) {
          ...
          return (
            !minDate ||
            plugin.add(plugin.newDate(inst.drawDate), -7, "d").getTime() >=
              minDate.getTime()
          );
        },
        date: function (inst) {
      ...
      prevDay: {
        ...
        enabled: function (inst) {
          ...
          return (
            !minDate ||
            plugin.add(plugin.newDate(inst.drawDate), -1, "d").getTime() >=
              minDate.getTime()
          );
        },
        date: function (inst) {
...
monthsOffset: function(date) { // Always start on the quarter
  ...
  alert('Selected ' + dates);
    ...
    formatDate: function (format, date, settings) {
      ...
      settings = settings || {};
      var dayNamesShort =
        settings.dayNamesShort || this.defaultOptions.dayNamesShort;
      var dayNames = settings.dayNames || this.defaultOptions.dayNames;
      var monthNamesShort =
        settings.monthNamesShort || this.defaultOptions.monthNamesShort;
    ...
    parseDate: function (format, value, settings) {
      ...
      settings = settings || {};
      var shortYearCutoff =
        settings.shortYearCutoff || this.defaultOptions.shortYearCutoff;
      shortYearCutoff =
    ...
    determineDate: function (
      ...
      dateSpec =
        typeof dateSpec === "undefined"
          ? defaultDate
          : typeof dateSpec === "string"
          ? offsetString(dateSpec)
          : typeof dateSpec === "number"
          ? isNaN(dateSpec) || dateSpec === Infinity || dateSpec === -Infinity
            ? defaultDate
            : plugin.add(plugin.today(), dateSpec, "d")
          : plugin.newDate(dateSpec);
      return dateSpec;
    },
...
var doy = $.datepick.dayOfYear(new Date(2014, 12-1, 25)) */
    dayOfYear: function (year, month, day) {
      ...
      return (
        Math.floor((date.getTime() - newYear.getTime()) / plugin._msPerDay) + 1
      );
    },
...
var week = $.datepick.iso8601Week(new Date(2014, 12-1, 25)) */
    iso8601Week: function (year, month, day) {
      ...
      return (
        Math.floor(Math.round((time - checkDate) / plugin._msPerDay) / 7) + 1
      );
    },
...
$.datepick.newDate(2014, 12, 25) */
    newDate: function (year, month, day) {
      return !year
        ? null
        : year.getFullYear
        ? this._normaliseDate(new Date(year.getTime()))
        ...
        date.setTime(
          plugin
            .newDate(
              year,
              month + 1,
              Math.min(date.getDate(), this.daysInMonth(year, month + 1))
            )
            .getTime()
        );
      }
    ...
    _postAttach: function (elem, inst) {
      if (inst.inline) {
        inst.drawDate = plugin._checkMinMax(
          plugin.newDate(
            inst.selectedDates[0] || inst.get("defaultDate") || plugin.today()
          ),
          inst
        );
    ...
    _optionsChanged: function (elem, inst, options) {
      ...
      inst.drawDate = plugin.newDate(
        this._checkMinMax(
          (inst.options.defaultDate
      ...
      if (inst.options.autoSize && !inst.inline) {
        ...
        if (dateFormat.match(/[DM]/)) {
          ...
          date.setMonth(
            findMax(
              inst.options[
                dateFormat.match(/MM/) // Longest month
                  ? "monthNames"
                  : "monthNamesShort"
              ]
            )
          ...
          date.setDate(
            findMax(
              inst.options[
                dateFormat.match(/DD/) // Longest day
                  ? "dayNames"
                  : "dayNamesShort"
              ]
            ) +
        ...
        inst.elem.attr(
          "size",
          plugin.formatDate(dateFormat, date, inst.getConfig()).length
        );
      }
    },
    ...
    _updateInput: function (elem, keyUp) {
      ...
      if (!$.isEmptyObject(inst)) {
        var value = "";
        var altValue = "";
        var sep = inst.options.multiSelect
          ? inst.options.multiSeparator
          : inst.options.rangeSeparator;
...
$(selector).datepick('setDate', [date1, date2, date3]) */
          ...
          switch (inst.selectedDates.length) {
            ...
            case 2:
              inst.selectedDates[1] =
                inst.selectedDates[0].getTime() >
                inst.selectedDates[1].getTime()
    ...
    isSelectable: function (elem, date) {
      ...
      date = plugin.determineDate(
        date,
        inst.selectedDates[0] || this.today(),
        null,
        inst.options.dateFormat,
        inst.getConfig()
      );
    ...
    _isSelectable: function (elem, date, onDate, minDate, maxDate) {
      var dateInfo =
        typeof onDate === "boolean"
          ...
          : !$.isFunction(onDate)
          ? {}
          : onDate.apply(elem, [date, true]);
      return (
        dateInfo.selectable !== false &&
        (!minDate || date.getTime() >= minDate.getTime()) &&
        (!maxDate || date.getTime() <= maxDate.getTime())
      );
    },
    ...
    showMonth: function (elem, year, month, day) {
      ...
      if (
        ...
        (typeof day !== "undefined" ||
          inst.drawDate.getFullYear() !== year ||
          inst.drawDate.getMonth() + 1 !== month)
      ) {
        inst.prevDate = plugin.newDate(inst.drawDate);
        var show = this._checkMinMax(
          typeof year !== "undefined"
            ? plugin.newDate(year, month, 1)
            : plugin.today(),
          inst
        );
        inst.drawDate = plugin.newDate(
    ...
    _checkMinMax: function (date, inst) {
      ...
      date =
        minDate && date.getTime() < minDate.getTime()
          ? plugin.newDate(minDate)
          : date;
      date =
        maxDate && date.getTime() > maxDate.getTime()
          ? plugin.newDate(maxDate)
          : date;
      return date;
    },
    ...
    retrieveDate: function (elem, target) {
      ...
      return $.isEmptyObject(inst)
        ...
        : this._normaliseDate(
            new Date(
              parseInt(target.className.replace(/^.*dp(-?\d+).*$/, "$1"), 10)
            )
          );
    },
    ...
    _generateContent: function (elem, inst) {
      ...
      inst.drawDate = this._checkMinMax(
        inst.drawDate || inst.get("defaultDate") || plugin.today(),
        inst
      );
      var drawDate = plugin._applyMonthsOffset(
        plugin.newDate(inst.drawDate),
        inst
      );
      // Generate months
      ...
      var addCommand = function (type, open, close, name, classes) {
        ...
        var date = inst.options.commandsAsDateFormat
    ...
    _generateMonth: function (elem, inst, year, month, renderer, first) {
      ...
      plugin.add(
        drawDate,
        -leadDays - (fixedWeeks && drawDate.getDay() === firstDay ? 7 : 0),
        "d"
      );
      ...
      for (var week = 0; week < numWeeks; week++) {
        ...
        for (var day = 0; day < 7; day++) {
          ...
          if (inst.options.rangeSelect && inst.selectedDates.length > 0) {
            selected =
              drawDate.getTime() >= inst.selectedDates[0] &&
          ...
          days += this._prepare(renderer.day, inst).replace(
            ...
            (selectable ? '<a href="javascript:void(0)"' : "<span") +
              ...
              ((drawDate.getDay() || 7) < 6
                ? ""
                : " " + renderer.weekendClass) +
              (drawDate.getMonth() + 1 === month
                ? ""
                : " " + renderer.otherMonthClass) +
              (drawDate.getTime() === today.getTime() &&
              drawDate.getMonth() + 1 === month
                ? " " + renderer.todayClass
                : "") +
              (drawDate.getTime() === inst.drawDate.getTime() &&
              drawDate.getMonth() + 1 === month
    ...
    _generateMonthSelection: function (
      ...
      // Months
      var monthNames =
        inst.options["monthNames" + (monthHeader.match(/mm/i) ? "" : "Short")];
      ...
      for (var m = 1; m <= 12; m++) {
        if (
          (!minDate ||
            plugin.newDate(year, m, plugin.daysInMonth(year, m)).getTime() >=
              minDate.getTime()) &&
          (!maxDate ||
            plugin.newDate(year, m, 1).getTime() <= maxDate.getTime())
        ) {
          selector +=
            '<option value="' +
            m +
      ...
      if (yearRange === "any") {
        ...
        if (start.getTime() < end.getTime()) {
          start = (
            minDate && minDate.getTime() > start.getTime() ? minDate : start
          ).getFullYear();
          end = (
            maxDate && maxDate.getTime() < end.getTime() ? maxDate : end
          ).getFullYear();
          ...
          start = (
            maxDate && maxDate.getTime() < start.getTime() ? maxDate : start
          ).getFullYear();
          end = (
            minDate && minDate.getTime() > end.getTime() ? minDate : end
          ).getFullYear();
